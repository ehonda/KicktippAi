...
public sealed class RunRepeatedMatchSliceSettings : RunExperimentSettingsBase
{
    [CommandOption("--batch-count")]
    [Description("Optional number of post-warmup batches per fixture. Defaults to 3")]
    [DefaultValue(3)]
    public int BatchCount { get; set; } = 3;

    [CommandOption("--parallelism")]
    [Description("Maximum number of repeated-match fixture workflows to run concurrently. Defaults to 5")]
    [DefaultValue(5)]
    ...
    public override ValidationResult Validate()
    {
        ...
        if (Parallelism < 1)
        {
            return ValidationResult.Error("--parallelism must be at least 1");
        }
