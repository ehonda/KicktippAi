...
/// <summary>
/// Command for generating HTML snapshots from Kicktipp for test fixtures.
/// Snapshots are saved unencrypted - use Encrypt-Fixture.ps1 to encrypt them before committing.
/// </summary>
public class GenerateSnapshotsCommand : AsyncCommand<GenerateSnapshotsSettings>
{
    public override async Task<int> ExecuteAsync(CommandContext context, GenerateSnapshotsSettings settings)
    {
        var logger = LoggingConfiguration.CreateLogger<GenerateSnapshotsCommand>();
        ...
        try
        {
            ...
            AnsiConsole.MarkupLine("[green]Generate-snapshots command initialized[/]");
            ...
            WarnIfNotGitignored(outputPath, logger);

            // Create snapshot client using the authenticated HttpClient
            ...
            // Fetch and save each page type
            await AnsiConsole.Status()
                .StartAsync("Fetching snapshots...", async ctx =>
                {
                    // 1. Tabellen (standings)
                    ctx.Status("Fetching tabellen...");
                    var tabellenContent = await snapshotClient.FetchStandingsPageAsync(settings.Community);
                    ...
                    var tippabgabeContent = await snapshotClient.FetchTippabgabePageAsync(settings.Community);
                    ...
                    var bonusContent = await snapshotClient.FetchBonusPageAsync(settings.Community);
                    ...
                    var spielinfoPages = await snapshotClient.FetchAllSpielinfoAsync(settings.Community);
            ...
            AnsiConsole.MarkupLine("[dim]  1. Review the HTML files to ensure they contain expected data[/]");
            AnsiConsole.MarkupLine("[dim]  2. Run Encrypt-Fixture.ps1 to encrypt them for committing[/]");
            AnsiConsole.MarkupLine("[dim]  3. Copy encrypted files to tests/KicktippIntegration.Tests/Fixtures/Html/[/]");

            return 0;
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error executing generate-snapshots command");
    ...
    private static async Task SaveSnapshotAsync(string outputPath, string fileName, string content)
    {
        var filePath = Path.Combine(outputPath, fileName);
        await File.WriteAllTextAsync(filePath, content);
    }

    private static void WarnIfNotGitignored(string outputPath, ILogger logger)
    {
        // Get the directory name relative to current directory
        ...
        // Simple heuristic: warn if the directory name doesn't suggest it's ignored
