...
public sealed class FirebaseContextSourceCycleRepository : IBundesligaContextSourceCycleRepository
{
    ...
    public Task<BundesligaContextSourceReceipt> RecordReceiptAsync(BundesligaContextSourceReceiptRequest request, CancellationToken cancellationToken = default)
    {
        ...
        return _db.RunTransactionAsync(async transaction =>
        {
            ...
            if (request.SelectionDisposition == BundesligaContextSourceSelectionDisposition.MetadataUnchanged)
            {
                ...
                var priorSnapshot = priorIdentity == request.Identity
                    ? targetSnapshot
                    : await transaction.GetSnapshotAsync(ReceiptReference(priorIdentity, request.Source, request.ConsumerLaneId));
                ...
                if (priorSelection is null
                    || priorSelection.CommunityContext != priorReceipt.Request.CommunityContext
                    || priorSelection.SelectedSnapshotId != priorReceipt.Request.SelectedSnapshotId
                    || priorSelection.SelectedOrigin != priorReceipt.Request.SelectedOrigin
                    || priorSelection.RatedAt != priorReceipt.Request.SourceDates.RatedAt
                    || priorSelection.MembershipCapturedAt != priorReceipt.Request.SourceDates.MembershipCapturedAt
                    || priorSelection.MembershipEffectiveAt != priorReceipt.Request.SourceDates.MembershipEffectiveAt
                    || priorSelection.EnrichmentCapturedAt != priorReceipt.Request.SourceDates.EnrichmentCapturedAt)
                    throw new InvalidDataException("STATE_CONFLICT");
    ...
    private static string ReceiptSemanticJson(BundesligaContextSourceReceiptRequest value) => JsonSerializer.Serialize(value, JsonOptions);
