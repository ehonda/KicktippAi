...
```
   ...
   `SAMPLING_CUTOFF` must be exactly Europe/Berlin local midnight two calendar days after `MODEL_CUTOFF_YYYY_MM_DD`; for Luna this is `2026-02-18T00:00:00 Europe/Berlin (+01)` after the `2026-02-16` official cutoff. Historical preparation resolves the exact producer-era seven-document route for every completed post-cutoff fixture at `startsAt -12h`, forms the complete context-eligible pool, and only then applies the requested seed once. Never retry or substitute a seed to bypass missing context. Treat an identity, scope, timestamp, or hash error as a provenance failure, not an ineligible fixture.

   Inspect the emitted `historicalCompatibility` before sync or spend. It must bind eligibility policy `bundesliga-2025-26-completed-after-sampling-cutoff-all-7-context-documents-at-or-before-starts-at-minus-12h-v1`, a positive `eligibleFixtureCount`, and a lowercase sorted-newline `eligibleFixtureIdsHash`. For Luna's documented cutoff in `pes-squad`, require the audited count `109` and hash `6ecb182489b97f9ea389374183f0ef7cfe632ddfba341ea72aa354647593b415`; a mismatch is a stop condition, not a reason to try another seed.

   The run must use hosted prompt `kicktippai/bundesliga-2026-27/predict-one-match` version `2`, label `production`, and relative `startsAt -12:00:00`. Record the prompt route as `Langfuse Bundesliga match v2; Bundesliga 2025/26 7-document legacy-id-hash-v1 context`. State that the row is a preseason cost proxy that may understate live 2026/27 input cost because the historical contract has seven documents while the live contract has eleven. Do not make a prediction-quality claim from a cost row.
