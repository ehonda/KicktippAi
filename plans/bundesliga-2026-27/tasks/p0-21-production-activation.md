...
- [ ] Before any initial prediction in each production community, explicitly publish rosters from ADR-0050's exact pinned DuckDB SHA/revision/date with `--require-launch-coverage`; record v2 headed snapshot/disposition/document versions, totals at or above 464 ages / 464 positions / 450 values, exactly 18 final `Team Accumulated` rows, and no regression from the accepted arena validation. This publication alone is not prediction-posting or schedule authority.
