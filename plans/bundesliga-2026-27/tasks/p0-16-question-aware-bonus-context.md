...
- 2026-08-21: [ADR-0038](../decisions/0038-bound-bonus-context-by-question-policy.md) accepts the explicit `Champion`, `Relegation`, `TopScorer`, `Coach`, and `Unknown` policy, fail-closed multi-category ambiguity, exact option/member identity routing, and the immutable whole-selection budget. German and English phrase variants use letter/digit boundaries; longer-token false positives and the unrelated `Champions League` phrase remain `Unknown`. Options cannot classify a question.
...
- 2026-08-21: The resolved result validates exact manifest document names/order/hashes, selected-name equality, unique disjoint selected/excluded identities, recomputed size, and the effective budget. Prediction observations receive category, selected/excluded documents, bytes/tokens, and both budgets. A two-question command regression proves each metadata object is rebuilt so a top-scorer roster/reason cannot leak into the following unknown question. The ADR-0037 persisted manifest stays byte/content focused and unchanged.
- 2026-08-21: Focused Release validation passed: Core policy/result/budget `29/29` (`0.715s` final rerun), Firebase provider `32/32` (`1.279s`), Bonus command family `94/94` (`11.250s`), and OpenAI telemetry/composer/bonus prediction `34/34` (`1.235s`). The fixed-set provider test independently binds the estimator to the exact prompt renderer.
- 2026-08-21: Four affected full Release assemblies ran in parallel and passed: Core `219/219` (`4.585s` test time), Firebase `278/278` (`1m57.794s`), OpenAI `220/220` (`5.477s`), and Orchestrator `990/990` (`2m00.348s`). The final `dotnet build KicktippAi.slnx --configuration Release --no-restore` passed with `0` errors in `51.65s` after a preceding restored build populated all fresh-worktree assets; existing analyzer, obsolete-API, nullability, and `SSH.NET` advisory warnings remain outside P0-16.
