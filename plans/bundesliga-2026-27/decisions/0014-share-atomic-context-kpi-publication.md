...
Publication uses a retry-safe Firestore transaction and requires `expectedPreviousSnapshotId` to equal the current head, including `null` for the first publication. The adapter reads and validates the head envelope, applies Core's pure CAS precondition before reading any snapshot graph, then returns `Unchanged` when target equals current, `Reactivated` when a valid target metadata row already exists, or `Published` otherwise. The transaction reads its immutable snapshot and exact payload versions, any existing target snapshot, and the maximum existing version for every requested name before performing writes. Maximum-version allocation deliberately uses the established `competition` + `communityContext` + `documentName` + descending `version` query (not `publicationSet`), so legacy, unheaded, and other-set rows are considered without a new Firestore index. It validates scope, metadata document identity, the definition's ordered key set, entry hashes, snapshot ID, and payload identity.

Unchanged documents reuse the exact version referenced by the current valid head. Changed documents allocate above every existing version for that competition, community, kind, and name, including legacy, other-set, and unheaded versions. After all reads, each transaction attempt obtains one timestamp shared by every newly-created payload and its metadata row; reused, reactivated, and unchanged rows retain their original metadata. The transaction creates changed payload rows and new immutable metadata, then switches the single head. A transaction, validation, or concurrency failure cannot advance the head or expose a partial set through the snapshot read boundary.
