...
- Before estimating or running cost-sensitive live experiments, verify every planned model is represented in `src/OpenAiIntegration/CostCalculationService.cs`. Missing or stale model pricing must be added from the official OpenAI pricing source at <https://developers.openai.com/api/docs/pricing.md> and covered by focused cost-calculation tests before the run.
- For reasoning-heavy model configs, run a one-item preflight with the intended prompt source and reasoning effort before the full live run. If the model returns no output text because reasoning consumed the output cap, increase `--max-output-tokens` explicitly, verify the cap is below the official model limit, and record the cap in run names and experiment docs.
