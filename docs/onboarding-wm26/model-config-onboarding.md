...
The scheduled self-contained `ehonda-ai-arena` entrypoints remain active
alongside that production path because they use
`community_context: "ehonda-ai-arena"` and their own model-specific Kicktipp
posting credentials. In addition to `gpt-5-nano` / `minimal`, the
`ehonda-ai-arena` `gpt-5.5` / `none`, `gpt-5.5` / `xhigh`,
`gpt-5.4-nano` / `none`, and `o3` / `medium` entries below are now scheduled
self-contained comparison or onboarding paths. They are not the selected WM26
