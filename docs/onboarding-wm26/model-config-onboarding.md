...
| WM26 production predictions: `rabetrabauken2026` reference, `ehonda-ai-arena` secondary copy-posting target | `fifa-world-cup-2026` | TBD; must be more capable than the dev/testing fallback | TBD | Reference context collection is wired in `.github/workflows/rabetrabauken2026-context-collection.yml`; model-specific matchday/bonus workflows are intentionally not created yet | Context workflow is manual-only; prediction workflows are not activated | Missing until production model and reasoning effort are selected. |
