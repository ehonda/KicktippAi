...
| --- | --- | --- | --- | --- | --- | --- |
| `ehonda-dev-wm26` manual testing | `fifa-world-cup-2026` | `gpt-5-nano` with `minimal` reasoning | Langfuse `kicktippai/wm26/predict-one-match` and `kicktippai/wm26/predict-bonus`, label `latest`; fallback model `wm26` | Code defaults in `src/Orchestrator/Commands/Shared/PredictionServiceCommandSupport.cs` and `src/Orchestrator/Infrastructure/CompetitionResolver.cs`; docs in `docs/onboarding-wm26/README.md` | Manual dev commands only; no GitHub Actions schedule activated | Missing. `uv --cache-dir .uv-cache run python .agents/skills/estimate-experiment-cost-skill/scripts/experiment_cost_estimator.py estimate --counts 104 --model gpt-5-nano --reasoning-effort minimal` reported no matching base estimate row on 2026-05-31. |
