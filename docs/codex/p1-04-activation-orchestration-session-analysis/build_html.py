...
<main>
  ...
  <section class="panel" id="polling">
    ...
    <p>Pure wait responses carried {number(usage['total_tokens'])} tokens, 39.1% of root tokens. The 55-second cadence produced 144 no-change timeouts in 194 calls. By contrast, {long_early} of {long_calls} waits requested at 300 or 600 seconds returned early on events. Event-driven long waits preserved responsiveness while materially lowering poll frequency.</p>
    <p><strong>Recommendation:</strong> default active-agent waits to 300–600 seconds and rely on event-driven early return or owner steering. Use short waits only for a documented near-term deadline.</p>
  </section>
  ...
  <section class="panel" id="actions">
    ...
    <ol class="actions">
      ...
      <li><strong>Wait policy:</strong> move the default from 55 seconds to 300–600 seconds. The observed long waits already returned early on 25 of 41 calls.</li>
