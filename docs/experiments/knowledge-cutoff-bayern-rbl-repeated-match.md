...
| Sample size | 25 repeated predictions for the paired comparison; 100 repeated predictions for the `gpt-5.5 low` follow-up |
...
`gpt-5.5 low` follow-up used the same source match, selected-item hash, exact
evaluation time, and hosted prompt route, but a separate 100-repetition
manifest and `--batch-count 10`.
...
The `gpt-5.5 low` run is a single-run follow-up rather than part of the paired
25x statistical report. It repeats the same source match and evaluation setup on
a 100-repetition dataset.
...
follow-up, while it was absent from both `gpt-5.5 none` and `gpt-5-nano` in the
25x comparison.
...
from the original xhigh sample.
