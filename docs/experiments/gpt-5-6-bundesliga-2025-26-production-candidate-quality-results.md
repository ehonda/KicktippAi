...
**Status:** Evidence complete; production-model and arena-participant selection
remain Owner decisions in P0-06
...
The browser-friendly generated report is
[published under the experiment-analysis tree](../../experiment-analysis/repeated-match-slices/pes-squad/all-matchdays-after-20260217t230000z/random-10x20-seed-20260821-gpt-5-6-production-candidate-quality/gpt-5-6-production-candidate-quality-2026-08-26t12-03-30z.analysis.report.html).
...
with `production` membership, shared run stamp `2026-08-26t12-03-30z`, cap
`10000`, batch count `7`, parallelism `5`, and evaluation time `startsAt -12h`.
The prompt was not changed by this experiment.
...
| Rank | Configuration | Dataset run ID | Total / avg repetition total | Point buckets `0/2/3/4` | Observed tiers | Max output | Observed cost | 493-call estimate | Dataset-run start UTC | Local wrapper duration |
|---:|---|---|---:|---:|---|---:|---:|---:|---|---:|
...
| 2 | Sol / `high` | `ebeb2218-7f77-4bdc-a415-61a23dfb566d` | `528 / 26.4` | `58/10/20/112` | Flex 69; Standard fallback 131 | 1339 | `$1.165440000000` | `$3.350033600000` | `2026-08-26T12:07:18Z` | not retained |
| 3 | Sol / `medium` | `82bd2eb7-fda4-4247-a924-f37b6b90f790` | `510 / 25.5` | `66/3/20/111` | Flex 200 | 487 | `$0.434544600000` | `$2.916193600000` | `2026-08-26T12:25:46Z` | `414.081s` |
| 4 | Luna / `medium` | `63463a23-2f99-4c48-990b-c5493482265f` | `490 / 24.5` | `70/10/10/110` | Flex 200 | 401 | `$0.025842800000` | `$0.170089930000` | `2026-08-26T15:09:24Z` | `409.105s` |
| 5 | Sol / `none` | `913f2985-e1aa-4111-90d9-22410536641b` | `460 / 23.0` | `79/3/18/100` | Flex 200 | 17 | `$0.198269600000` | `$2.487283600000` | `2026-08-26T12:35:00Z` | not retained |
| 6 | Terra / `xhigh` | `b78e0fca-f785-4d51-994f-5c8f73f8edce` | `448 / 22.4` | `80/6/20/94` | Flex 186; Standard fallback 14 | 1490 | `$0.519026300000` | `$2.040181900000` | `2026-08-26T12:44:28Z` | not retained |
| 7 | Terra / `medium` | `fecfe01c-4109-47d2-95fe-13aeafbc202e` | `404 / 20.2` | `92/4/20/84` | Flex 181; Standard fallback 19 | 353 | `$0.277898000000` | `$1.600475200000` | `2026-08-26T12:53:14Z` | `409.704s` |
| 8 | Terra / `none` | `08806884-75df-4c43-8cb0-46c01fa127bb` | `342 / 17.1` | `107/5/20/68` | Flex 182; Standard fallback 18 | 17 | `$0.134951500000` | `$1.252022800000` | `2026-08-26T13:01:43Z` | `403.031s` |
| 9 | Luna / `none` | `56d2750d-51dc-4a52-8dba-310256a70f72` | `312 / 15.6` | `115/8/12/65` | Flex 199; Standard fallback 1 | 17 | `$0.010064270000` | `$0.125202280000` | `2026-08-26T15:19:09Z` | `650.309s` |

The wrapper duration is the exact local runner interval where its payload-safe
terminal record was retained. The normalized report records the exact
dataset-run start for all nine runs, so missing wrapper durations do not weaken
...
seed `20260406`. The Friedman statistic is `128.25862822193102`, with
`p=6.493535233483119e-24`. The omnibus gate therefore permits interpretation
of the 36 corrected pairwise comparisons; 26 are significant after both Holm
correction and the omnibus gate.

The generated rank order is descriptive. In the exploratory nine-run family:

- Sol/`xhigh` is first on the point estimate, but its difference from
  Sol/`high` is not Holm-significant (`+1.4`, adjusted `p=0.1920`), nor is its
  difference from Sol/`medium` (`+2.3`, adjusted `p=0.0671`).
- Among the original completed configurations, Sol/`high`, Sol/`medium`, and
  ...
  differences is significant in this nine-run corrected family.
- Sol/`xhigh` does exceed Luna/`medium` in this sample (`+3.3`, adjusted
  `p=0.0080`), but that comparison is post-hoc and data-dependent.
- Luna/`medium` combines an average repetition total of `24.5` with a
  ...
  `26.4` at `$3.350033600000`, and exploratory Sol/`xhigh` reaches `27.8` at
  `$4.204649100000`. This is evidence for the Owner's quality/cost tradeoff,
  not an automatic winner rule.

No inference including Sol/`xhigh` is confirmatory. A fresh preregistered sample
would be required for a confirmatory Sol/`xhigh` comparison. These 2025/26
post-cutoff fixtures are the only available preseason outcomes, and the small
ten-fixture surface may not generalize to the 2026/27 season. Repetitions reduce
model-output variance on the same fixtures; they do not create 200 independent
football matches.
...
The ceiling was never close to binding; unused allowance is not evidence that
...
P0-23 supplies evidence only. P0-06 remains open for the Owner to select the
production model, effort, cap, service-tier/fallback policy, season ceiling,
and arena participants and to record those choices in a new Accepted ADR. No
production workflow, community prediction, participant topology, or schedule
