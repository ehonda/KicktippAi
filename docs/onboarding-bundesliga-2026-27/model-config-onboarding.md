...
| `production-primary` — `pes-squad` / `schadensfresse` generation and compatible copies | `bundesliga-2026-27` | `gpt-5.6-sol` | `xhigh` | `10000` | Match v3; bonus v1; exact names/hashes below | Existing Flex-first/Standard-fallback policy; USD 35 season orientation is planning-only and not enforced | Owner-selected; manual cycles and payload-safe audit are green for `pes-squad`, `relaxdays-tippt`, and the arena copy; these ready rows are included in ADR-0053's match-only lane while `schadensfresse` remains excluded |
...
| `arena-challenger-luna-none` / `validation-luna-none` — `ehonda-ai-arena` | `bundesliga-2026-27` | `gpt-5.6-luna` | `none` | `10000` | Match v3; bonus v1 | Self-contained arena context; existing Flex-first/Standard-fallback policy | Owner-admitted challenger and retained plumbing identity; manual context, match, and Owner-approved forced index-0 bonus recovery are green and payload-audited; included in ADR-0053's match-only lane; do not repeat recovery |
...
P0-19 owns leaf-workflow creation. Every manual entry provides the exact ledger values as explicit inputs and does not rely on command model, reasoning, cap, prompt-label, or prompt-version defaults. ADR-0053 schedules only P0-21's strict outer ready-row matchday lane.
