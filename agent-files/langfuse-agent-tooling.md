...
```powershell
npx skills add langfuse/skills --skill "langfuse" --global --yes
npx skills update langfuse --global --yes
```

The explicit global scope prevents the installer from creating a repo-local `.agents/skills/langfuse` copy and `skills-lock.json`. For agent-specific installs, add `--agent "<agent-id>"`. Cursor users can alternatively install the Langfuse Cursor plugin, which includes the skill.
...
If `langfuse` is missing or stale, install or update it with `npm i -g langfuse-cli@latest` after the user approves the global install, then verify the installed version with `langfuse --version`. Avoid `npx langfuse-cli ...` for routine work because it fetches executable package code at runtime and can create unnecessary credential-exposure risk when paired with `--env`.
...
4. Add the authenticated Langfuse MCP server when an MCP-capable client needs direct access to project data. Use the same host as `LANGFUSE_HOST` and a Basic auth token made from `LANGFUSE_PUBLIC_KEY:LANGFUSE_SECRET_KEY`.
...
The authenticated MCP server now covers Langfuse resources beyond prompt management, including observations and related project data. Both read and write tools are enabled by default. Consult the canonical [Langfuse MCP reference](https://mcp.reference.langfuse.com) for the current tool list, and configure client approvals or an allowlist when you want read-only behavior. For command-capable agents, Langfuse recommends the official skill and CLI over the authenticated MCP server.
...
- Use the authenticated MCP server when the client cannot run the CLI or when MCP-native project access is useful. Prefer the official skill plus the installed CLI for command-capable agents and discover the current API surface with `langfuse api __schema`.
