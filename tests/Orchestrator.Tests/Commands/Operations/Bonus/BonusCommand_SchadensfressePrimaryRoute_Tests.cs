...
public sealed class BonusCommand_SchadensfressePrimaryRoute_Tests : BonusCommandTests_Base
{
    [Test]
    ...
    [Test]
    public async Task Canonical_schadensfresse_CL_set_resolves_rules_context_then_stops_before_prompt_or_prediction_activity()
    {
        var (factory, bindings, documents) = CreateCanonicalTypedContextFactory();
        var context = CreateBonusCommandApp(
            openBonusQuestions: CreateCanonicalQuestions(),
        ...
        context.KicktippClient.Verify(client => client.PlaceBonusPredictionsAsync(It.IsAny<string>(), It.IsAny<Dictionary<string, BonusPrediction>>(), It.IsAny<bool>()), Times.Never);
    }

    [Test]
    public async Task Unknown_open_target_question_fails_before_typed_context_factory_activity()
    {
        var unknown = CreateCanonicalQuestions();
        unknown[0] = unknown[0] with { KicktippQuestionId = "9999999999" };
        var context = CreateBonusCommandApp(openBonusQuestions: unknown);
    ...
    [Test]
    public async Task Conflicting_open_target_subcompetition_fails_before_typed_context_factory_activity()
    {
        var conflicting = CreateCanonicalQuestions();
        conflicting[0] = conflicting[0] with
        {
            BundesligaSeasonSubcompetition = BundesligaSeasonSubcompetition.Bundesliga
        };
        var context = CreateBonusCommandApp(openBonusQuestions: conflicting);
    ...
    [Test]
    public async Task Every_incomplete_or_mutated_target_question_set_fails_before_typed_context_or_forbidden_activity()
    {
        var variants = CreateInvalidQuestionSets();
        foreach (var questions in variants)
        {
            ...
            context.KicktippClient.Verify(client => client.PlaceBonusPredictionsAsync(
                It.IsAny<string>(), It.IsAny<Dictionary<string, BonusPrediction>>(), It.IsAny<bool>()), Times.Never);
        }
    }

    [Test]
    public async Task Missing_binding_or_wrong_immutable_document_stops_before_prompt_or_prediction_activity()
    {
        ...
        var missingContext = CreateBonusCommandApp(openBonusQuestions: CreateCanonicalQuestions(), firebaseServiceFactory: missingFactory);
        ...
        await AssertNoForbiddenTargetActivity(missingContext);
        ...
        var documentContext = CreateBonusCommandApp(openBonusQuestions: CreateCanonicalQuestions(), firebaseServiceFactory: documentFactory);
        var documentExit = await documentContext.App.RunAsync(CanonicalArguments());
        await Assert.That(documentExit).IsEqualTo(1);
        await AssertNoForbiddenTargetActivity(documentContext);
    }

    [Test]
    public async Task Stale_future_or_over_budget_typed_context_fails_before_prompt_or_prediction_activity()
    {
        var variants = new[]
        {
        ...
        foreach (var variant in variants)
        {
            ...
            var context = CreateBonusCommandApp(openBonusQuestions: CreateCanonicalQuestions(), firebaseServiceFactory: factory);
            var exitCode = await context.App.RunAsync(CanonicalArguments());
            await Assert.That(exitCode).IsEqualTo(1);
            await AssertNoForbiddenTargetActivity(context);
        }
    }

    [Test]
    public async Task Target_spelling_or_season_or_context_drift_fails_before_every_dependency()
    {
        var argumentVariants = new[]
        {
            new[] { "bonus", "test-model", "--community", "Schadensfresse", "--community-context", "schadensfresse", "--competition", CompetitionIds.Bundesliga2026_27 },
            new[] { "bonus", "test-model", "--community", "schadensfresse", "--community-context", "", "--competition", CompetitionIds.Bundesliga2026_27 },
            new[] { "bonus", "test-model", "--community", "schadensfresse", "--community-context", "schadensfresse", "--competition", "wm26" }
        };
    ...
    public async Task Target_command_propagates_a_pre_cancelled_token_before_dependency_activity()
    {
        ...
        var command = new BonusCommand(
            context.Console, context.FirebaseServiceFactory.Object, context.KicktippClientFactory.Object,
            context.OpenAiServiceFactory.Object, context.ContextProviderFactory.Object, context.CredentialLoader.Object,
            NullLogger<BonusCommand>.Instance);
        await Assert.That(() => command.ExecuteWithSettingsAsync(new BonusSettings
        {
    ...
    [Test]
    public async Task Target_command_propagates_the_exact_open_question_cancellation_without_downstream_activity()
    {
        var context = CreateBonusCommandApp();
        ...
        var command = new BonusCommand(context.Console, context.FirebaseServiceFactory.Object, context.KicktippClientFactory.Object,
            context.OpenAiServiceFactory.Object, context.ContextProviderFactory.Object, context.CredentialLoader.Object, NullLogger<BonusCommand>.Instance);
    ...
    private static IReadOnlyList<List<BonusQuestion>> CreateInvalidQuestionSets()
    {
        ...
        [
            ...
            [canonical[0] with { BundesligaSeasonSubcompetition = BundesligaSeasonSubcompetition.DfbPokal }, canonical[1], canonical[2]]
        ];
    }

    private static string[] CanonicalArguments() =>
    [
        "bonus", "test-model", "--community", "schadensfresse", "--community-context", "schadensfresse",
        "--competition", CompetitionIds.Bundesliga2026_27
    ];

    private static async Task AssertNoForbiddenTargetActivity(BonusCommandTestContext context)
    {
        await Assert.That(context.OpenAiServiceFactory.Invocations).Count().IsEqualTo(0);
        await Assert.That(context.ContextProviderFactory.Invocations).Count().IsEqualTo(0);
        await Assert.That(context.PredictionRepository.Invocations).Count().IsEqualTo(0);
